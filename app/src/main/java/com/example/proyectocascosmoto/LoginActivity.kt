...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnIniciar.setOnClickListener {
            ...
            //ALERTO SI HAY CAMPOS EN BLANCO, DEBEN ESTAR AMBOS CAMPOS COMPLETOS PARA AVANZAR AL ELSE
            if (nombreUsuario.isEmpty() || etPass.text.toString().isEmpty()) {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    //Dispatchers.IO -> contexto en el cual es adecuado para operaciones E/S, como las operaciones con bases de datos

                    try {

                        //COMPRUEBO LOS USUARIOS REGISTRADOS EN LA BASE DE DATOS

                        val bdd = AppDataBase.getDatabase(applicationContext)
                        val checkUsuario = bdd.usuarioDao().getNombre(nombreUsuario) // Realizar operaciones de base de datos en Dispatchers.IO

                        if (checkUsuario == null) {
                            // Manejo del caso "Usuario no registrado"
                            withContext(Dispatchers.Main) {
                                Toast.makeText(this@LoginActivity, "Usuario no registrado", Toast.LENGTH_SHORT).show()
                            }
                        } else if (!checkUsuario.Contraseña.equals(passwordUsuario)) {

                            // Manejo del caso "Contraseña incorrecta"
                            withContext(Dispatchers.Main) {
                    ...
                    } catch (e: Exception) { // Manejo de excepciones

                        // msj Por consola
                         Log.e("MiTag", "Error en la corrutina: ${e.message}", e)
